...
function accInitBottone() { 
 function tenta() { 
  ...
  if (box && accRilevaStaff()) { box.style.display = ''; return true; } 
  return false; 
 } 
 if (tenta()) return; 
 if (window.HxHFramework && window.HxHFramework.utilities && window.HxHFramework.utilities.waitFor) { 
  ...
  // Poller di riserva: ~6s, poi si arrende (il box resta nascosto) 
  var n = 0; 
  var t = setInterval(function(){ n++; if (tenta() || n > 24) clearInterval(t); }, 250); 
 } 
} 
...
function scegliModalita(modalita) { 
 ...
 // Reset visivo di tutti i box 
 var boxes = [btnNuova, btnModifica, btnAccrediti]; 
 for (var bi = 0; bi < boxes.length; bi++) { 
  if (boxes[bi]) { boxes[bi].style.borderColor = '#3B8686'; boxes[bi].style.background = 'rgba(0,0,0,0.2)'; } 
 } 
