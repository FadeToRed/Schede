...
// I dati testuali del form vengono salvati in localStorage a ogni 
// modifica, e ripristinati quando la persona rientra nella pagina. 
// Le immagini (crop base64) sono ESCLUSE di proposito: pesano troppo 
// e saturerebbero lo spazio disponibile. 
// ============================================================ 
var BOZZA_KEY = 'af_bozza_scheda'; 
var _bozzaTimer = null; 

// Raccoglie tutti i campi con id che inizia per "campo-". 
// Le immagini vivono in altri contenitori (non "campo-"), quindi 
// restano automaticamente fuori dal salvataggio. 
function bozzaCampi() { 
 var root = document.getElementById('form-container') || document; 
 var tutti = root.querySelectorAll('[id^="campo-"]'); 
 var out = []; 
 for (var i = 0; i < tutti.length; i++) { 
  var el = tutti[i]; 
  var tag = (el.tagName || '').toLowerCase(); 
  if (tag === 'input' || tag === 'select' || tag === 'textarea') out.push(el); 
 } 
 return out; 
} 

// Salva lo stato corrente dei campi. Debounce per non scrivere a 
// ogni singolo tasto. 
function bozzaSalva() { 
 ...
 _bozzaTimer = setTimeout(function() { 
  try { 
   ...
   for (var i = 0; i < campi.length; i++) { 
    var el = campi[i]; 
    if (el.type === 'checkbox' || el.type === 'radio') dati[el.id] = el.checked ? 1 : 0; 
    else dati[el.id] = el.value; 
   } 
   var pacchetto = { modalita: stato.modalita, palette: stato.palette, campi: dati, ts: Date.now() }; 
...
// Ripristina i campi dalla bozza salvata (se presente e compatibile 
// con la modalità corrente). Va chiamata DOPO che il form è nel DOM. 
function bozzaRipristina() { 
 var pacchetto = null; 
 try { pacchetto = JSON.parse(localStorage.getItem(BOZZA_KEY)); } catch (e) { pacchetto = null; } 
 if (!pacchetto || !pacchetto.campi) return false; 
 // Non mescolare bozze di modalità diverse (es. nuova vs modifica) 
 if (pacchetto.modalita && stato.modalita && pacchetto.modalita !== stato.modalita) return false; 
 ...
 for (var id in dati) { 
  ...
  if (el.type === 'checkbox' || el.type === 'radio') el.checked = !!dati[id]; 
  else el.value = dati[id]; 
 } 
 return true; 
} 

// Collega il salvataggio a ogni campo del form. 
function bozzaCollega() { 
 var campi = bozzaCampi(); 
 for (var i = 0; i < campi.length; i++) { 
  var el = campi[i]; 
  if (el._bozzaBound) continue; 
  el._bozzaBound = true; 
  el.addEventListener('input', bozzaSalva); 
  el.addEventListener('change', bozzaSalva); 
 } 
} 
...
function costruisciForm() { 
 ...
 accordionInit(); 
 // Ripristina l'eventuale bozza salvata, poi rende alcuni campi 
 // dipendenti coerenti con i valori appena reinseriti. 
 var ripristinata = bozzaRipristina(); 
 if (ripristinata) { 
  if (isNuova) { aggiornaRazza(true); aggiornaStatNuova(); } 
  else { aggiornaRazza(false); aggiornaCompetenze(); } 
 } 
