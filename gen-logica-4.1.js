...
function cropModalImgCaricata() {
 ...
 setTimeout(function() {
  ...
  // ── DIAGNOSTICA TEMPORANEA — rimuovere dopo aver capito il bug ──
  try {
   console.log('[CROP DEBUG] id=' + _cropModal.id
    + ' | slot=' + slotW + 'x' + slotH + ' (AR ' + (slotW/slotH).toFixed(3) + ')'
    + ' | nat=' + img.naturalWidth + 'x' + img.naturalHeight
    + ' | disp=' + Math.round(dispW) + 'x' + Math.round(dispH) + ' (AR ' + (dispW/dispH).toFixed(3) + ')'
    + ' | canvasOffset=' + canvas.offsetWidth + 'x' + canvas.offsetHeight
    + ' | box%: w=' + cw.toFixed(1) + ' h=' + ch.toFixed(1) + ' x=' + cx.toFixed(1) + ' y=' + cy.toFixed(1)
    + ' | boxAR_px=' + ((cw/100*canvW)/(ch/100*canvH)).toFixed(3));
   // Dopo il render, misura il box VERO a schermo
   setTimeout(function(){
    var b = document.getElementById('crop-box');
    if (b) console.log('[CROP DEBUG] box reale a schermo: '
      + Math.round(b.offsetWidth) + 'x' + Math.round(b.offsetHeight)
      + ' (AR ' + (b.offsetWidth/b.offsetHeight).toFixed(3) + ')'
      + ' | canvas reale: ' + canvas.offsetWidth + 'x' + canvas.offsetHeight);
   }, 30);
  } catch(e) {}
  // ── FINE DIAGNOSTICA ──
