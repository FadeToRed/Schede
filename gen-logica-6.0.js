...
function costruisciProgressione() { 
 ...
 // Campo EXP da accreditare 
 html += riga2( 
  '<div style="margin-bottom:14px;"><label style="'+STILE_LABEL+'">EXP da accreditare</label><input type="number" id="acc-exp-delta" min="0" step="1" value="0" style="'+STILE_INPUT+'"></div>', 
  '<div style="margin-bottom:14px; display:flex; align-items:flex-end;"><button onclick="accCalcolaProgressione()" style="width:100%; background:#292354; color:#CFF09E; border:1px solid #3B8686; padding:11px 18px; border-radius:6px; cursor:pointer; font-family:\'Montserrat\'; font-size:1em;"><i class="fa-solid fa-calculator"></i> Calcola progressione</button></div>' 
 ); 
 // Anteprima risultato 
...
function costruisciStatAccrediti() { 
 ...
 var html = ''; 
 html += '<p style="color:#8FBEBA; font-size:0.88em; margin-bottom:10px; font-style:italic;">I punti disponibili derivano dai level up calcolati sopra. Vanno distribuiti <b>tutti</b> per poter registrare l\'accredito.</p>'; 
...
function costruisciValoriDiretti() { 
 var html = ''; 
 html += riga2( 
  '<div style="margin-bottom:14px;"><label style="'+STILE_LABEL+'">Jenny</label><input type="text" id="acc-jenny" placeholder="Es. 15000" style="'+STILE_INPUT+'"></div>', 
  '<div style="margin-bottom:14px;"><label style="'+STILE_LABEL+'">HC</label><input type="text" id="acc-hc" placeholder="Es. 0" style="'+STILE_INPUT+'"></div>' 
 ); 
 html += '<div style="margin-bottom:14px;"><label style="'+STILE_LABEL+'">Nen (%)</label><input type="number" id="acc-nen" min="0" max="100" step="1" placeholder="Es. 20" style="'+STILE_INPUT+'"></div>'; 
...
function accModificaStat(stat, delta) { 
 ...
 // Somma già distribuita su tutte le stat 
 var totDistribuito = accTotDistribuito(); 
 if (delta > 0 && totDistribuito + delta > budget) return; // non sforare il budget 
 if (delta < 0 && extra + delta < 0) return;               // non scendere sotto il valore importato 
 extra += delta; 
 exEl.value = extra; 
...
function accAggiornaContatore() { 
 var budget = stato.progressione ? stato.progressione.puntiDaDistribuire : 0; 
 var rimasti = budget - accTotDistribuito(); 
 var el = document.getElementById('acc-punti-counter'); 
 if (el) { 
  el.textContent = rimasti + ' punti da distribuire' + (rimasti === 0 && budget > 0 ? ' — completato ✓' : ''); 
  el.style.color = (rimasti === 0) ? '#A8DBA8' : '#F9C6C6'; 
 } 
 return rimasti; 
} 
...
function importaScheda() { 
 ...
 if (stato.modalita === 'accrediti') { 
  accCatturaBase(temp); 
  accPopolaDopoImport(); 
  alert('Scheda importata! Inserisci l\'EXP da accreditare e premi "Calcola progressione".'); 
  return; 
 } 
...
function accPopolaDopoImport() { 
 ...
 // Valori diretti precompilati con gli attuali 
 var jn = document.getElementById('acc-jenny'); 
 var soldiRaw = null; 
 // Jenny/HC li rileggo dal temp non è comodo qui: lasciamo vuoti come "nuovo valore" 
 // Nen attuale 
 // (Nen e Jenny/HC restano da inserire manualmente come nuovo valore assoluto) 
 accAggiornaContatore(); 
} 
...
function raccogliDatiAccrediti() { 
 ...
 function numVal(id, fallback) { 
  ...
  var v = e.value.replace(/\./g,'').replace(/[^\d]/g,'').trim(); 
  return v !== '' ? v : fallback; 
 } 
 var soldiRaw = entry('Soldi:'); 
 var jennyAtt = '0', hcAtt = '0'; 
 if (soldiRaw) { 
  var spSoldi = soldiRaw.split('/'); 
  if (spSoldi.length >= 2) { 
   jennyAtt = spSoldi[0].replace('Jenny','').replace(/\./g,'').replace(/[^\d]/g,'').trim() || '0'; 
   hcAtt = spSoldi[1].replace('HC','').replace(/[^\d]/g,'').trim() || '0'; 
  } 
 } 
 var jenny = numVal('acc-jenny', jennyAtt); 
 var hc    = numVal('acc-hc', hcAtt); 

 var nenAtt = 0; 
 if (root) { 
  var bcs = root[metodo]('div'); 
  for (var k = 0; k < bcs.length; k++) { 
   if (bcs[k].className === 'barra-card') { 
    var le = bcs[k].querySelector ? bcs[k].querySelector('.barra-card-label') : null; 
    var pe = bcs[k].querySelector ? bcs[k].querySelector('.barra-card-pct') : null; 
    if (le && pe && le.textContent.trim() === 'Nen') nenAtt = parseInt(pe.textContent.replace('%','')) || 0; 
   } 
  } 
 } 
 var nenEl = document.getElementById('acc-nen'); 
 var nen = (nenEl && nenEl.value.trim() !== '') ? (parseInt(nenEl.value) || 0) : nenAtt; 
