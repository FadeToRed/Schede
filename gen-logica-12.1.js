...
function accAggiornaFedina() { 
 ...
 // Taglia sempre come select modificabile (nessun readonly forzato). 
...
function costruisciStatusTaglia() { 
 ...
 for (var ct = 0; ct < CLASSIFICAZIONI_TAGLIA.length; ct++) { 
  if (CLASSIFICAZIONI_TAGLIA[ct] === 'E') continue; 
...
function raccogliDatiAccrediti() { 
 ...
 if (fedina === 'Ricercato') { 
  ...
  var readonlyVis = ctr && ctr.style.display !== 'none'; 
  classTaglia = readonlyVis ? 'E' : (cte ? cte.value : 'E'); 
  var vtRaw = vte ? (parseInt(vte.value.replace(/\./g,'').replace(/[^\d]/g,'')) || VALORE_TAGLIA_BASE['E']) : VALORE_TAGLIA_BASE['E']; 
